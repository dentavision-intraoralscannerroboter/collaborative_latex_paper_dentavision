...
Examples of the output produced by the C++ UDP/GVSP implementation during the data capture experiments are provided in Appendix~\ref{app:udp-output}. The four captures show representative outputs obtained during the communication and packet-analysis experiments.
...
reconstruction. This characteristic makes the Middlebury scenes, and particularly
\textit{Plastic}, a more relevant approximation of the low-texture conditions
that may occur on dental surfaces than the initially used Palm Desert dataset.
...
Although an integration with the other components of the DentaVision system could not be completed, the reconstruction stage was designed to provide a clean interface for the subsequent 3D registration stage. The rectified stereo image pair generated by the simulator, together with the relevant camera information, can be used by the selected stereo reconstruction method to generate a 3D point cloud in \code{.ply} format. This output is intended to serve as the input to the subsequent registration stage, allowing the reconstruction component to be integrated into the overall pipeline without requiring changes to its internal processing.